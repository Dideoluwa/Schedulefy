      ...
      <div className='calendar-container1'>
        <Routes>
          <Route path='/' element={(loggedIn && <Calender loading={loading} onClickFor={navigateForwardHAndler} onClick={navigateBackHAndler} disabled={isActive} show={show} dates={dates} timer={timer} />) || (!loggedIn && <Navigate to='/auth' />)}>
            ...
            <Route path='form' element={(loggedIn && <Form purpose={purpose} setPurpose={setPurpose} name={name} setName={setName} phoneNumber={phoneNumber} setPhoneNumber={setPhoneNumber} email={email} setEmail={setEmail} onClick={navigateForwardHAndler} setIsActive={setIsActive} />) || (!loggedIn && <Navigate to='/auth' />)} />
          </Route>
